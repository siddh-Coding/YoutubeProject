...
import YoutubeCommentScrapper as yt
...
# st.set_page_config(page_title='Jatin_Agrawal_20BCS6606', initial_sidebar_state='auto')
st.set_page_config(page_title='Youtube Comments Sentimenter', page_icon=None, layout="centered",
...
if youtube_link:
    ...
    channel_id = yt.get_channel_id(video_id)
    if video_id:
        st.sidebar.write("The video ID is:", video_id)
        csv_file = yt.save_video_comments_to_csv(video_id)
        delete_non_matching_csv_files(directory_path, video_id)
        st.sidebar.write("Comments saved to CSV!")
        st.sidebar.download_button(label="Download Comments", data=open(
            csv_file, 'rb').read(), file_name=os.path.basename(csv_file), mime="text/csv")

        # using fn
        channel_info = yt.get_channel_info(yt.youtube, channel_id)
